...
- `scan.run_anomaly_scan` / `ScanResult` — the single-call orchestration surface:
  anchored/expanding walk-forward refit, disjoint OOS folds concatenated into one
  zero-look-ahead score/flag series, plus the JSON-safe descriptive summary.
...
- Partitioned test suite (unit / parity / property / regression / integration)
  ...
  `pure_noise`): 192 tests, coverage ≈ 93 % (gate ≥ 85 %).
- `docs/DESIGN.md` and ADRs 0001–0005 (train-only fit, PCA-AE-not-torch, the
...
- Jaccard ≈ **0.50** (modest), proxy precision ≈ **0.04** (low), proxy recall ≈
  **0.32**. Flags are **diagnostic, not tradable** — there is no ground-truth
  label, so no alpha is claimed.
