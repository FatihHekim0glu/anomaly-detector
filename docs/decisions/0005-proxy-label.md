...
- **Related:** [ADR-0004](0004-no-groundtruth-descriptive.md) (no ground truth, so
...
- A day is a "proxy anomaly" when its return, standardized by a **strictly
  ...
  use, so it never peeks at the day it normalizes), is more than 3 sigma from zero.
...
On the seeded synthetic series this yields proxy precision about `0.04` and recall
about `0.32`: the detectors and the naive proxy overlap on a *core* but are far
from identical, which is exactly the point. If precision were high, the detectors
